...
class EngineCamera():
    def __init__(self, main, fixed_upon, settings:dict={}):
        ...
        self.camera: list = [0, 0, self.main.native_resolution[0], self.main.native_resolution[1]]
    ...
    def camera_fixed(self):
        ...
        self.camera[0] = fixed_pos[0] - distance[0] // 2
        self.camera[1] = fixed_pos[1] - distance[1] // 2

    def camera_abstract(self):
        distance:  tuple = self.main.native_resolution
        fixed_pos: tuple = (self.fixed_upon.pos[0] - int(distance[0] / 2), self.fixed_upon.pos[1] - int(distance[1] / 2))

        dx = fixed_pos[0] - self.camera[0]
        dy = fixed_pos[1] - self.camera[1]

        # Apply lerp
        new_camera_pos: tuple = lerp(point_1=(self.camera[0], self.camera[1]), point_2=fixed_pos, factor=self.lerp_factor)
        self.camera[0] = new_camera_pos[0]
        self.camera[1] = new_camera_pos[1]

        # Apply acceleration
        if abs(dx) > self.x_speed_max:
            self.camera[0] += self.x_speed_max if dx > 0 else -self.x_speed_max
        else:
            self.camera[0] += dx * self.x_accel_rate

        if abs(dy) > self.y_speed_max:
            self.camera[1] += self.y_speed_max if dy > 0 else -self.y_speed_max
        else:
            self.camera[1] += dy * self.x_accel_rate
