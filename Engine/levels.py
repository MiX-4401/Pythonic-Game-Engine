...
class Level(EngineLevel):
    ...
    def create_object(self, type:str, properties:dict, polygons:list, pos:list, name:str):
        ...
        if type == "ball":
            ...
            object:      Ball = Ball(main=self.main, surfaces=surfaces, normals=normals, properties=properties, polygons=polygons, pos=[pos[0], pos[1]-tilesize[1]], name=name)
        if type == "player":
            spritesheet: list = [x for x in self.main.sprites.sprites["player"].values()]
            surfaces:    list = [x[0] for x in spritesheet[1::]]
            normals:     list = [x[1] for x in spritesheet[1::]]
            object:      Player = Player(main=self.main, surfaces=surfaces, normals=normals, properties=properties, polygons=polygons, pos=[pos[0], pos[1]-tilesize[1]*2], name=name)
